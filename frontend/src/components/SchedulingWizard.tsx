...
export const SchedulingWizard = ({
  ...
  const generatePreviewDates = () => {
    ...
    if (config.frequency === 'monthly') {
      first.setDate(config.dayOfMonth || 1);
      if (first <= now) {
      ...
      if (diffDays === 0 && first <= now) {
        diffDays = 7;
      }
      first.setDate(first.getDate() + diffDays);
    }

    const stepDays = config.frequency === 'biweekly' ? 14 : config.frequency === 'weekly' ? 7 : 0;

    for (let i = 0; i < 3; i++) {
      const run = new Date(first);
      if (config.frequency === 'monthly') {
        run.setMonth(first.getMonth() + i);
        // Correct for months shorter than the target day
        const monthEnd = new Date(run.getFullYear(), run.getMonth() + 1, 0).getDate();
        run.setDate(Math.min(config.dayOfMonth || 1, monthEnd));
      } else {
