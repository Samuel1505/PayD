...
const mockTransactions = [
  { id: "TXN-1001", employeeName: "Wilfred G.", category: "Salary", date: "2026-02-15", amount: "5000.00", currency: "USDC", status: "Completed" },
  { id: "TXN-1002", employeeName: "Chinelo A.", category: "Bonus", date: "2026-02-10", amount: "1500.00", currency: "USDC", status: "Completed" },
  { id: "TXN-1003", employeeName: "Jane Smith", category: "Expense", date: "2026-02-05", amount: "250.00", currency: "XLM", status: "Pending" },
  { id: "TXN-1004", employeeName: "Emeka N.", category: "Salary", date: "2026-01-31", amount: "4200.00", currency: "EURC", status: "Completed" },
  { id: "TXN-1005", employeeName: "Fatima K.", category: "Expense", date: "2026-01-28", amount: "80.00", currency: "USDC", status: "Failed" },
  { id: "TXN-1006", employeeName: "Wilfred G.", category: "Bonus", date: "2026-01-15", amount: "1000.00", currency: "USDC", status: "Completed" },
];
...
export default function TransactionHistory() {
  ...
  const activeFilters = useMemo(() => {
    ...
    selectedCategories.forEach(c => filters.push({ type: 'category', value: c, label: `Category: ${c}` }));
    selectedEmployees.forEach(e => filters.push({ type: 'employee', value: e, label: `Employee: ${e}` }));
    if (dateRange.start) filters.push({ type: 'dateStart', value: dateRange.start, label: `From: ${dateRange.start}` });
    if (dateRange.end) filters.push({ type: 'dateEnd', value: dateRange.end, label: `To: ${dateRange.end}` });
    return filters;
  ...
  const removeFilter = (filter: { type: string; value: string }) => {
    switch (filter.type) {
      ...
      case 'category': setSelectedCategories(prev => prev.filter(c => c !== filter.value)); break;
      case 'employee': setSelectedEmployees(prev => prev.filter(e => e !== filter.value)); break;
      case 'dateStart': setDateRange(prev => ({ ...prev, start: '' })); break;
      case 'dateEnd': setDateRange(prev => ({ ...prev, end: '' })); break;
    }
  };
  ...
  const filteredTransactions = useMemo(() => {
    return mockTransactions.filter(txn => {
      const matchSearch = txn.id.toLowerCase().includes(searchTerm.toLowerCase()) || txn.employeeName.toLowerCase().includes(searchTerm.toLowerCase());
      const matchCategory = selectedCategories.length === 0 || selectedCategories.includes(txn.category);
      const matchEmployee = selectedEmployees.length === 0 || selectedEmployees.includes(txn.employeeName);
      ...
      {isFilterExpanded && (
        <div className="bg-[#16161a] border border-zinc-800 rounded-xl p-6 mb-8 shadow-xl animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Date Range */}
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Date Range</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Categories</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Employees</label>
      ...
      {activeFilters.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-6 p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-xl">
          ...
          {activeFilters.map((filter) => (
            <span key={`${filter.type}-${filter.value}`} className="flex items-center gap-1.5 bg-zinc-800 text-xs px-2.5 py-1 rounded-md text-zinc-300 border border-zinc-700">

              {filter.label}
              <button onClick={() => removeFilter(filter)} className="text-zinc-500 hover:text-red-400 transition-colors">
                <X size={12} />
              </button>
            </span>
          ))}
          <button onClick={clearAllFilters} className="text-xs text-blue-400 hover:text-blue-300 underline ml-auto font-medium">
            Clear all
          </button>
        </div>
      )}
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead className="bg-[#111115] sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70">Txn ID</th>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70">Date</th>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70">Employee</th>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70">Category</th>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70 text-right">Amount</th>
                <th className="p-4 text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800/70">Status</th>
              </tr>
            </thead>
            <tbody>
              {filteredTransactions.length > 0 ? (
                filteredTransactions.map((txn, idx) => (
                  <tr key={txn.id} className={`border-b border-zinc-800/30 hover:bg-zinc-800/20 transition-colors ${idx % 2 === 0 ? 'bg-transparent' : 'bg-zinc-900/10'}`}>
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-zinc-500">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <button onClick={clearAllFilters} className="text-blue-400 hover:text-blue-300 text-sm mt-2 transition-colors">Clear filters</button>
                    </div>
                  </td>
                </tr>
              )}
